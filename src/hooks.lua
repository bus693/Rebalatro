...
function create_card(_type, area, legendary, _rarity, skip_materialize, soulable, forced_key, key_append)
    if area == G.pack_cards and (key_append == 'ar1' or key_append == 'ar2') then
        ...
        if should_have_spectrals then
            if pseudorandom('omen_globe_mod') > 0.75 then
                _type = "Spectral"
                key_append = 'ar2'
            else
                _type = "Tarot"
...
SMODS.Atlas({ key = 'blueseal1', path = 'blueseal1rounds.png', px = 71, py = 95, prefix_config = {key = false} })
SMODS.Atlas({ key = 'blueseal2', path = 'blueseal2rounds.png', px = 71, py = 95, prefix_config = {key = false} })
SMODS.Atlas({ key = 'blueseal3', path = 'blueseal3rounds.png', px = 71, py = 95, prefix_config = {key = false} })
